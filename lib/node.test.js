...
describe('node.ts', function () {
    describe('Node', function () {
        ...
        it('Setting/getting buffer creates a copy', function () {
            ...
            node.setBuffer(buf);
            chai_1.expect(buf === node.getBuffer()).to.be.false; // Objects not equal, so copy.
        ...
        describe('.read(buf, off, len, pos)', function () {
            it('Simple one byte read', function () {
                var node = new node_1.Node(1);
                node.write(Buffer.from([1, 2, 3]));
                var buf = Buffer.allocUnsafe(1);
                node.read(buf, 0, 1, 1);
                chai_1.expect(buf.equals(Buffer.from([2]))).to.be.true;
            });
        });
    });
});
