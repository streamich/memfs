...
function dataToStr(data, encoding) {
  ...
  else if (data instanceof Uint8Array) return buffer_1.bufferFrom(data).toString(encoding);
...
function dataToBuffer(data, encoding) {
  ...
  else if (data instanceof Uint8Array) return buffer_1.bufferFrom(data);
  else return buffer_1.bufferFrom(String(data), encoding);
}
...
function validateBufferRange(buffer, offset, length) {
  if (offset === void 0) {
    offset = 0;
  }
  if (length === void 0) {
  ...
  if (offset < 0) throw new RangeError('offset must be a non-negative number');
  if (offset >= buffer.length) throw new RangeError("offset must be less than buffer's length");
  if (length < 0) throw new RangeError('length must be a non-negative number');
  if (offset + length > buffer.length)
    throw new RangeError("(offset + length) must not be bigger than buffer's length");
}
