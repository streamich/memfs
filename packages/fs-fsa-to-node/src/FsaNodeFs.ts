import * as optHelpers from '@jsonjoy.com/fs-node';
import * as util from '@jsonjoy.com/fs-node';
import { Buffer } from '@jsonjoy.com/fs-node-builtins/lib/internal/buffer';
import { FsPromises } from '@jsonjoy.com/fs-node';
import { pathToLocation } from './util';
import { ERRSTR, MODE, AMODE } from '@jsonjoy.com/fs-node-utils';
...
export class FsaNodeFs extends FsaNodeCore implements FsCallbackApi, FsSynchronousApi, FsCommonObjects {
  ...
  public readonly createWriteStream: FsCallbackApi['createWriteStream'] = (
    ...
    if (optionsObj.autoClose) {
      stream.once('close', () => void handle.then(file => this.close(file.fd, () => {})).catch(() => {}));
    }
    return stream;
  };
